...
#ifndef TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_ZRPC_ZRPC_SESSION_H_
#define TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_ZRPC_ZRPC_SESSION_H_
...
// computation on potentially remote sets of devices.
// This is a thin wrapper around tensorflow::zrpc::MasterService.
//
// Multiple threads must synchronize their accesses to a single
// session.
class ZrpcSession : public Session
{
protected:
    explicit ZrpcSession(const SessionOptions &options);

public:
    static Status Create(const SessionOptions &options, std::unique_ptr<ZrpcSession> *out_session);
    // Resets the resource containers.
    static Status Reset(const SessionOptions &options, const std::vector<string> &containers);

    ~ZrpcSession() override;
    ...
    Status Run(const std::vector<std::pair<string, Tensor>> &inputs,
               const std::vector<string> &output_tensor_names, const std::vector<string> &target_node_names,
               std::vector<Tensor> *outputs) override;
    Status Run(const RunOptions &run_options, const std::vector<std::pair<string, Tensor>> &inputs,
               const std::vector<string> &output_tensor_names, const std::vector<string> &target_node_names,
...
private:
    ...
    Status RunHelper(const RunOptions &run_options, const std::vector<std::pair<string, Tensor>> &inputs,
    ...
    TF_DISALLOW_COPY_AND_ASSIGN(ZrpcSession);
};

} // namespace tensorflow

#endif // TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_ZRPC_ZRPC_SESSION_H_
