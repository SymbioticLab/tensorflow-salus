...
void EncodeRecvTensorResponseToByteBuffer(const RecvTensorResponse &proto, ::zmq::MultiPartMessage &result)
{
    size_t len = proto.ByteSize();
    result->clear();
    auto &slice = result.emplace_back(len);
    proto.SerializeWithCachedSizesToArray(slice.data<uint8>());
}
...
static void EncodeSkeleton(const Tensor &val, io::ProtoEncodeHelper *e)
{
    ...
    for (int d = 0; d < ndims; d++) {
        ...
        tensor_shape_bytes += 2 + // TensorShapeProto dim tag + varintlength of submessage
...
void EncodeTensorToByteBuffer(bool is_dead, const Tensor &val, ::zmq::MultiPartMessage &result)
{
    ...
    if (!DataTypeCanUseMemcpy(val.dtype())) {
        ...
        uint32 overall_tensor_proto_bytesize =
            (e_skeleton.size() + VarLengthEncodingSize(TensorProto::kTensorContentFieldNumber, tdata.size()));
        string header; // All of RecvTensorRequest except the tensor() field
        response.AppendToString(&header);

        size_t expected_size = (header.size() + VarLengthEncodingSize(RecvTensorResponse::kTensorFieldNumber,
        ...
        // (B1) & (B2)
        e.WriteVarlengthBeginning(RecvTensorResponse::kTensorFieldNumber, overall_tensor_proto_bytesize);
        // (C)
        e.WriteRawBytes(StringPiece(e_skeleton.data(), e_skeleton.size()));
        // (D1) & (D2)
        e.WriteVarlengthBeginning(TensorProto::kTensorContentFieldNumber, tdata.size());
        ...
        result->clear();
        {
            ...
            auto &s0 = result.emplace_back(slice_len);
            memcpy(s0.data(), e.data(), e.size());
            if (!tensor_data_is_large) {
                // (E)
                memcpy(s0.data<uint8>() + e.size(), tdata.data(), tdata.size());
            }
        }

        if (tensor_data_is_large) {
            // Encode the actual tensor data by pointing to the backing store,
            // by directly point the message part data pointer to tensor data.
            //
            // (E) Encode tensor data, but by sharing backing store

            TensorReference *ref = new TensorReference(val);
            result->emplace_back(const_cast<char *>(tdata.data()), tdata.size(),
                                 [](void *, void *hint) {
                                     TensorReference *ref = static_cast<TensorReference *>(hint);
                                     ref->Unref();
                                     delete ref;
                                 },
                                 ref);
